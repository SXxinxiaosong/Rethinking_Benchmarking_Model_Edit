...
Just return the letters "A" or "B", with no text around it.
    ...
    logging.debug(f"生成的 llm_score: {llm_score}")
    return llm_score
...
class EditTool():
    ...
    def single_edit(self, prompt, target_new, knowledge=None, locality=False, vanilla_genration=True,
        ...
        logging.debug(f"生成的 prompt: {prompt}")
        prompt_tok = self.tok(
            prompt,
            return_tensors="pt",
        ).to(self.device)
        ...
        logging.debug(f"生成的 gen_content: {gen_content}")                
    ...
    if edit_scene=="sequential":
        ...
        for i, request in enumerate(tqdm(requests, total=len(requests))):
            ...
            if "prompt" in request:
                ...
                logging.debug(f"related_knowledge: {related_knowledge}")
